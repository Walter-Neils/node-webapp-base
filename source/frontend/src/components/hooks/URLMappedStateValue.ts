...
export function useURLMappedStateValue<T>(
	configuration: URLMappedValueConfiguration<T>,
): [T, (value: T) => void] {
	...
	const initializeValue = React.useCallback(() => {
		if (configuration.initializationBehaviour === 'load-from-url') {
			...
			if (value === null) {
				if (
					configuration.initializationValueMissingBehaviour === 'null'
				) {
					return null;
				} else if (
					configuration.initializationValueMissingBehaviour ===
					'throw'
				) {
					throw new Error(
						`URL parameter ${configuration.key} is missing`,
					);
				}
			} else {
		...
		configuration.initializationBehaviour,
		configuration.valueMode,
		configuration.key,
		urlParams,
	]);
	...
	const liveState = React.useRef({
		syncFromURLDisabled:
			configuration.initializationBehaviour === 'load-from-url',
	...
	React.useEffect(() => {
		...
		if (configuration.valueMode === 'json') {
			...
			const encoded = Buffer.from(JSON.stringify(currentValue)).toString(
				'base64',
			);
			urlParams.set(configuration.key, encoded);
		} else if (configuration.valueMode === 'string') {
			...
			urlParams.set(
				configuration.key,
				(currentValue as unknown as boolean) ? 'true' : 'false',
			);
		}

		const newUrl = `${location.pathname}?${urlParams.toString()}`;
		navigate(newUrl, {
			replace: configuration.navigationBehaviour === 'replace',
		});
	}, [currentValue]);
...
export class URLMappedValueIntentBuilder {
	...
	public setBase64Value(key: string, value: unknown) {
